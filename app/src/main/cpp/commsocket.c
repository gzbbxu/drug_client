...
int sckCliet_init(void **handle, int contime, int sendtime, int revtime, int nConNum) {
    ...
    if (handle == NULL || contime < 0 || sendtime < 0 || revtime < 0) {
        ret = Sck_ErrParam;
        ALOGE("func sckCliet_init() err: %d, check  (handle == NULL ||contime<0 || sendtime<0 || revtime<0)\n",
              ret);
        return ret;
    }
    ...
    if (tmp == NULL) {
        ret = Sck_ErrMalloc;
        ALOGE("func sckCliet_init() err: malloc %d\n", ret);
        return ret;
    }
...
int activate_nonblock(int fd) {
    ...
    if (flags == -1) {
        ret = flags;
        ALOGE("func activate_nonblock() err:%d", ret);
        return ret;
    }
...
int deactivate_nonblock(int fd) {
    ...
    if (flags == -1) {
        ret = flags;
        ALOGE("func deactivate_nonblock() err:%d", ret);
        return ret;
    }
...
//
int sckCliet_getconn(void *handle, const char *ip, int port, int *connfd) {

    int ret = 0;
    SckHandle *tmp = NULL;
    if (handle == NULL || ip == NULL || connfd == NULL || port < 0 || port > 65537) {
        ret = Sck_ErrParam;
        ALOGE("func sckCliet_getconn() err: %d, check  (handle == NULL || ip==NULL || connfd==NULL || port<0 || port>65537) \n",
              ret);
        return ret;
    }
    ...
    */
    ALOGE("before conn connect_timeout %d ",sockfd);
    ret = connect_timeout(sockfd, (struct sockaddr_in *) (&servaddr), (unsigned int) tmp->contime);
    ALOGE("after conn connect_timeout %d ",sockfd);
    if (ret < 0) {
        if (ret == -1 && errno == ETIMEDOUT) {
            ret = Sck_ErrTimeOut;
            return ret;
        } else {
            ALOGE("func connect_timeout() err:  %d\n", ret);
            ret = Sck_ErrTimeOut;
            return ret ;
        }
    }
    ALOGE("连接socket 成功 sockfd = %d ,ret = %d ",sockfd,ret);
    *connfd = sockfd;
...
int sckClient_send(void *handle, int connfd, unsigned char *data, int datalen) {
    ...
    if (ret == 0) {
        ...
        if (netdata == NULL) {
            ret = Sck_ErrMalloc;
            ALOGE("func sckClient_send() mlloc Err:%d\n ", ret);
            return ret;
        }
        ...
        ALOGE("客户端发送报文 writen>>  writed = %d  %d ",writed,datalen);
        if (writed <= (datalen + 4)) {
            if (netdata != NULL) {
                ALOGE("客户端发送报文 free ..");
                free(netdata);
...
int sckClient_rev(void *handle, int connfd, unsigned char *out, int *outlen) {
    ...
    if (handle == NULL || out == NULL) {
        ret = Sck_ErrParam;
        ALOGE("func sckClient_rev() timeout , err:%d \n", Sck_ErrTimeOut);
        return ret;
    }
    ALOGE("before read_timeout %d ", tmpHandle->revtime);
    ret = read_timeout(connfd, tmpHandle->revtime);
    ALOGE("after read_timeout  ");
    if (ret != 0) {
        if (ret == -1 || errno == ETIMEDOUT) {
            ret = Sck_ErrTimeOut;
            ALOGE("func sckClient_rev() timeout , err:%d \n", Sck_ErrTimeOut);
    ...
    if (ret == -1) {
        ALOGE("func readn() err:%d \n", ret);
        return ret;
    } else if (ret < 4) {
        ret = Sck_ErrPeerClosed;
        ALOGE("func readn() err peer closed:%d \n", ret);
        return ret;
    }
